...
const confirmRoomChange = document.getElementById('confirmRoomChange');
const cancelRoomChange = document.getElementById('cancelRoomChange');

// Oda değiştirme modalını aç
changeRoomButton.addEventListener('click', () => {
    roomChangeModal.classList.remove('hidden');
    newRoomInput.value = myRoom; // Mevcut oda adını göster
    newRoomInput.select(); // Metni seç
});

// Modalı kapat
cancelRoomChange.addEventListener('click', () => {
    roomChangeModal.classList.add('hidden');
});

// Odayı değiştir
confirmRoomChange.addEventListener('click', async () => {
    ...
    // Mevcut PeerConnection'ları kapat
    Object.keys(peerConnections).forEach(cleanupPeerConnection);
    
    // Audio elementlerini temizle
    while (remoteAudioContainer.firstChild) {
        remoteAudioContainer.removeChild(remoteAudioContainer.firstChild);
    }
    ...
    // Yeni oda bilgisini güncelle
    myRoom = newRoom;
    displayUsername.textContent = `${myUsername} (Oda: ${myRoom})`;

    // Yeni odaya bağlan
    connectToSignalingServer();

    // Ses durumunu kontrol et ve gerekirse yeniden başlat
    if (startButton.textContent === 'Sesi Durdur') {
        await startAudio();
    }
...
document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && !roomChangeModal.classList.contains('hidden')) {
        roomChangeModal.classList.add('hidden');
    }
});

// Modal dışına tıklama ile kapatma
roomChangeModal.addEventListener('click', (e) => {
    if (e.target === roomChangeModal) {
        roomChangeModal.classList.add('hidden');
    }
});
